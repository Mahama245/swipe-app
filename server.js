...
app.get("/api/me", authMiddleware, safe(async (req, res) => {
  ...
  // somehow isn't set, fix it here too rather than requiring a fresh login.
...
app.post("/api/webauthn/login-verify", safe(async (req, res) => {
  ...
  // Grants bootstrap-admin status here too, not just on password login —
  // an account that only ever signs in via Face ID/fingerprint shouldn't
  // be stuck without admin rights forever.
  await db.ensureBootstrapAdmin(user.username);
